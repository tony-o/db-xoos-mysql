...
my %queries =
  ...
  list-relations => "SELECT tc.table_name t1_name, kcu.column_name c1_name, ccu.table_name t2_name, ccu.column_name c2_name FROM information_schema.table_constraints AS tc JOIN information_schema.key_column_usage AS kcu ON tc.constraint_name = kcu.constraint_name AND tc.table_schema = kcu.table_schema JOIN information_schema.key_column_usage AS ccu ON ccu.constraint_name = tc.constraint_name AND ccu.table_schema = tc.table_schema WHERE tc.constraint_type = 'FOREIGN KEY' AND (tc.table_name = ? or ccu.table_name = ?)",
;
...
sub generate-structure (Str :$dsn?, :$db-conn?, Bool :$dry-run = False, :@tables? = []) is export {
  ...
  for @define-tables.map({ $_[0] }) -> $table {
    ...
    my @relations = $db.query(%queries<list-relations>, $table, $table).hashes;

    my %col-data;
    my %relations;
    for @columns -> $col {
      %col-data{$col<name>} = {
        type     => %translate{$col<type>.decode}//$col<type>.decode,
